...
//------------------------------------------------------------------------------
using _Pairii = pair<string, const Message*>;
void print_subject(const _Pairii& p,regex subject)
{
	for (auto pp = p.second->begin(); pp != p.second->end(); pp++)
	{
		smatch matches;
		if (regex_match(*pp, matches, subject))
		{
			cout << matches[1] << endl;
		}
	}
...
int main()
{
	...
	//pair<MCI, MCI> pp = sender.equal_range("z");
